...
// running real WASM FHE. It only accepts local dev chains (31337), not Sepolia.
const { RelayerCleartext } = require("@zama-fhe/sdk/cleartext");
const { chains: fheChains } = require("@zama-fhe/sdk/chains");
// chainId 31337 = hardhat/anvil — accepted by cleartext relayer
const hardhatFhe = fheChains[31337];
...
async function main() {
  ...
  // Chain 31337 is hardhat/anvil — the only chain cleartext mode allows.
  // Tests 1 and 2 run against this mock chain; Test 3 uses the real Sepolia RPC.
  const cleartext = new RelayerCleartext(hardhatFhe);
  ...
  // Verify the adapter by manually converting the rawResult and comparing
  const adaptedHandles = rawResult.encryptedValues.map((v) => hexToBytes(v));
  const adaptedProof = hexToBytes(rawResult.inputProof);

  assert(Array.isArray(adaptedHandles), "handles must be array");
  assert(adaptedHandles[0] instanceof Uint8Array, "handles[0] must be Uint8Array");
  assert(adaptedProof instanceof Uint8Array, "inputProof must be Uint8Array");
  assert(adaptedHandles[0].length === 32, `handle must be 32 bytes, got ${adaptedHandles[0].length}`);
  assert(adaptedProof.length > 0, "inputProof must have bytes");
  console.log("✓ toEncryptor adapter: Hex→Uint8Array conversion correct");
  console.log("  handle length:", adaptedHandles[0].length, "bytes");
  console.log("  inputProof length:", adaptedProof.length, "bytes");
  // Verify round-trip on same value
  const roundTripped = bytesToHex(adaptedHandles[0]);
  ...
  // Verify the adapter function itself works on a fresh call
  ...
  assert(adaptedResult.handles[0] instanceof Uint8Array, "adapter must return Uint8Array handle");
  assert(adaptedResult.inputProof instanceof Uint8Array, "adapter must return Uint8Array proof");
  console.log("✓ toEncryptor wraps relayer correctly (adapter call verified)");
